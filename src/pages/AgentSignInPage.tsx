...
export default function AgentSignInPage({ onAuth, onNav }: Props) {
  ...
  async function handleSignIn(e: React.FormEvent) {
    ...
    if (!profile) {
      setError('Account not found. Please contact your manager.');
      await supabase.auth.signOut();
      setLoading(false);
      return;
    }

    if (profile.role !== 'agent' && profile.role !== 'sales_manager' && profile.role !== 'admin' && profile.role !== 'worker') {
      setError('This portal is for agents only. Please use the standard sign-in.');
      await supabase.auth.signOut();
      setLoading(false);
      return;
    }
    ...
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex flex-col justify-between w-[420px] shrink-0 bg-[#0f172a] px-10 py-12">
        <div>
          ...
          <div className="text-[#94a3b8] text-[14px] leading-relaxed mb-10">
            Sign in to view your claims, track referrals, and monitor your earnings in real time.
          </div>

          <div className="space-y-4">
            {[
              { label: 'Track referrals', desc: 'See every claim from your referral link.' },
              { label: 'Monitor earnings', desc: 'Live view of your compensation totals.' },
              { label: 'Real-time updates', desc: 'Status changes the moment they happen.' },
            ].map(item => (
        ...
        <div className="text-[#334155] text-[11px]">
          ClaimVelo Ltd. · Agent & Partner Portal
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center bg-[#f8fafc] px-6 py-12">
        <div className="w-full max-w-[400px]">
          ...
          <div className="mb-7">
            <div className="text-[24px] font-extrabold text-[#0f172a] mb-1.5">Agent sign in</div>
            <div className="text-[13px] text-[#64748b]">
              Agent accounts are created by your manager. Contact them if you need access.
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-[#e2e8f0] shadow-sm p-7">
            <form onSubmit={handleSignIn} className="flex flex-col gap-4">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full py-2.5 rounded-lg font-semibold text-[13px] text-white border-none cursor-pointer transition-all disabled:opacity-60 bg-[#2563eb] hover:bg-[#1d4ed8] active:scale-[0.98] mt-1"
              >
